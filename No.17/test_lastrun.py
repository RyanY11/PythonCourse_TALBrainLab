...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\TAL\\内部学习\\Python2021\\No17PsychoPy\\test_lastrun.py',
...
image = visual.ImageStim(
    win=win,
    name='image', 
    image='C:\\\\Users\\\\TAL brainlab\\\\Pictures\\\\EpicFun\\\\screen_2020-07-10_16-53-02.png', mask=None,
...
brush = visual.Brush(win=win, name='brush',
   lineWidth=1.5,
   lineColor=(-0.2157, 0.1686, 0.8588),
   lineColorSpace='rgb',
   opacity=None,
   buttonRequired=True)
...
while continueRoutine:
    ...
    if key_resp.status == STARTED and not waitOnFlip:
        theseKeys = key_resp.getKeys(keyList=['y', 'n', 'left', 'right', 'space', 'enter'], waitRelease=False)
...
item_loop = data.TrialHandler(nReps=5.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='item_loop')
...
if thisItem_loop != None:
    for paramName in thisItem_loop:
        exec('{} = thisItem_loop[paramName]'.format(paramName))
...
for thisItem_loop in item_loop:
    ...
    _key_resp_2_allKeys = []
    # keep track of which components have finished
    itemComponents = [image, text_2, brush, key_resp_2]
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    itemClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    ...
    while continueRoutine:
        # get current time
        t = itemClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=itemClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        ...
        if brush.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > brush.tStartRefresh + 5.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if key_resp_2.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_2.getKeys(keyList=['y', 'n', 'left', 'right', 'space', 'enter'], waitRelease=False)
    ...
    for thisComponent in itemComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    item_loop.addData('image.started', image.tStartRefresh)
    item_loop.addData('image.stopped', image.tStopRefresh)
    item_loop.addData('text_2.started', text_2.tStartRefresh)
    item_loop.addData('text_2.stopped', text_2.tStopRefresh)
    item_loop.addData('brush.started', brush.tStartRefresh)
    item_loop.addData('brush.stopped', brush.tStopRefresh)
    # check responses
    if key_resp_2.keys in ['', [], None]:  # No response was made
        key_resp_2.keys = None
    item_loop.addData('key_resp_2.keys',key_resp_2.keys)
    if key_resp_2.keys != None:  # we had a response
        item_loop.addData('key_resp_2.rt', key_resp_2.rt)
    item_loop.addData('key_resp_2.started', key_resp_2.tStartRefresh)
    item_loop.addData('key_resp_2.stopped', key_resp_2.tStopRefresh)
    # the Routine "item" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 5.0 repeats of 'item_loop'
