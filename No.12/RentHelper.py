...
class RentTool(QWidget):
    ...
    def initUI(self):
        ...
        #btn.sizeHint()显示默认尺寸
        #openloacation.resize(openloacation.sizeHint())
    ...
    #定义爬虫核心模块
    def get_info(self, url, type, num):
        headers = {
        ...
        for item in item_a_title:
            ...
            if type == 1:
                for c in self.keys:
                        if c in name:
                            self.csv_writer.writerow([name, link])
                            self.items += 1
                            self.items_total += 1
            #使用排除关键词
            else:
    ...
    def getContent(self):
        ...
        f = open('租房信息检索结果.csv','w',encoding='utf-8-sig',newline="")
        self.csv_writer = csv.writer(f)
        # 构造表头
        self.csv_writer.writerow(['租房信息标题', '链接地址'])
        ...
        for i in range(all_page):
            self.items = 0
            self.get_info(url=urls[i], type=self.methodType, num=page_num[i])
